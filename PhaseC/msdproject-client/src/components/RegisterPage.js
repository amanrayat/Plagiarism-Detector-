...
class RegisterPage extends React.Component{
  ...
  constructor(){
    super();
    this.state = {userID:'', univID:'', name:'', userRole:'', password:'', email:'', successMessage: 'Not Saved'};
    this.handleClick = this.handleClick.bind(this)
  }

  handleClick() {
    console.log("Success from RegisterPage!")
    console.log('http://localhost:8080/team208/registerUser?studentDBid='+this.state.userID+'&userId='+this.state.univID+'&name='+this.state.name+'&userRole='+this.state.userRole+'&password='+this.state.password+'&email='+this.state.email)
    axios.get('http://localhost:8080/team208/registerUser?studentDBid='+this.state.userID+'&userId='+this.state.univID+'&name='+this.state.name+'&userRole='+this.state.userRole+'&password='+this.state.password+'&email='+this.state.email)
